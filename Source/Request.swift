...
public struct HTTPPair {
    ...
    */
    ...
    /**
     Protobuf type
     */
    var protobufData: ProtobufData? {
        return storeVal as? ProtobufData
    }
...
/**
Support for the ProtobufData type as an HTTPParameter.
*/
extension ProtobufData: HTTPParameterProtocol {
    public func createPairs(_ key: String?) -> [HTTPPair] {
        var collect = [HTTPPair]()
        collect.append(HTTPPair(key: key, value: self))
        return collect
...
extension URLRequest {
    ...
    */
    public mutating func appendParameters(_ parameters: HTTPParameterProtocol) -> Error? {
        ...
        } else if containsProtobuf(parameters) {
            return appendParametersAsProtobufData(parameters)
        } else {
    ...
    */
    ...
    /**
     send the parameters as a body of Protobuf
     -parameter parameters: The container Protobuf to convert and append to the  Body
     */
    public mutating func appendParametersAsProtobufData(_ parameters: HTTPParameterProtocol) -> Error? {
        var mutData = Data()
        for pair in parameters.createPairs(nil) {
            if let resp = pair.protobufData {
                if let data = resp.data {
                    mutData = data
                    break
                } else {
                    return HTTPProtobufError.noData
                }
            }
        }
        httpBody = mutData as Data
        var contentStr = "application/protobuf"
        if let charset = CFStringConvertEncodingToIANACharSetName(CFStringConvertNSStringEncodingToEncoding(String.Encoding.utf8.rawValue)) {
            contentStr += "; charset=\(charset)"
        }
        setValue(contentStr, forHTTPHeaderField: contentTypeKey)
    ...
    */
    ...
    public func containsProtobuf(_ parameters: HTTPParameterProtocol) -> Bool {
        for pair in parameters.createPairs(nil) {
            if let  _ = pair.protobufData {
                return true
            }
        }
        return false

...
open class HTTP {
    ...
    */
    @discardableResult open class func GET(_ url: String, parameters: HTTPParameterProtocol? = nil, headers: [String: String]? = nil,
        requestSerializer: HTTPSerializeProtocol = HTTPParameterSerializer(),
        timeoutInterval: TimeInterval? = nil, completionHandler: ((Response) -> Void)? = nil) -> HTTP? {
        guard let task = HTTP.New(url, method: .GET, parameters: parameters, headers: headers, timeoutInterval: timeoutInterval, requestSerializer: requestSerializer, completionHandler: completionHandler) else {return nil}
        task.run()
        return task
    }
    ...
    */
    ...
    Class method to run a POST request that handles the URLRequest and parameter encoding for you.
    */
    @discardableResult open class func POST(_ url: String, parameters: HTTPParameterProtocol? = nil, headers: [String: String]? = nil, requestSerializer: HTTPSerializeProtocol = HTTPParameterSerializer(), timeoutInterval: TimeInterval? = nil, completionHandler: ((Response) -> Void)? = nil) -> HTTP? {
        guard let task = HTTP.New(url, method: .POST, parameters: parameters, headers: headers, timeoutInterval: timeoutInterval, requestSerializer: requestSerializer, completionHandler: completionHandler) else {return nil}
        task.run()
        return task
    }
    ...
    */
    open class func New(_ url: String, method: HTTPVerb, parameters: HTTPParameterProtocol? = nil, headers: [String: String]? = nil, timeoutInterval: TimeInterval? = nil, requestSerializer: HTTPSerializeProtocol = HTTPParameterSerializer(), completionHandler: ((Response) -> Void)? = nil) -> HTTP? {
...
public class DelegateManager: NSObject, URLSessionDataDelegate, URLSessionDownloadDelegate {
    ...
    func removeTask(_ task: URLSessionTask) {
        taskMap.removeObject(key: task.taskIdentifier)
    }
    
    // add the response task
    func addResponseForTask(_ task: URLSessionTask) {
        if taskMap.object(key: task.taskIdentifier) == nil {
            taskMap.setObject(object: Response(), key: task.taskIdentifier)
        }
    }
    // get the response object for the task
    func responseForTask(_ task: URLSessionTask) -> Response? {
        return taskMap.object(key: task.taskIdentifier) as? Response
    }
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let hresponse = task.response as? HTTPURLResponse {
            resp.headers = hresponse.allHeaderFields as? [String: String]
